...
class JenkinsLancher(object):
    ...
    def update_war(self):
        if not os.path.exists(self.war_directory):
            os.makedirs(self.war_directory)
        ...
        if os.path.exists(self.war_path):
            ...
            subprocess.check_call([
                'wget',
                '-O',
                self.war_directory + '/jenkins.war',
    ...
    def update_config(self):
        ...
        config_source = resource_string('jenkinsflow.test.framework',
                                        'config.xml')
        try:
    ...
    def start(self, timeout=60):
        if not self.jenkins_url:
            ...
            os.environ['JENKINS_URL'] = 'http://localhost:%s/' % self.http_port
            test_dir = os.getcwd()
